...
import AddTerms from "../../admin/TermsAndConditions/AddTerms";
import AddPolicy from "../../admin/policy/AddPolicy";

const AdminPolicy = () => {
  const [selected, setSelected] = useState();
  const [isBusy, setIsBusy] = useState(false);
  const { data, loading, refetch } = useGetHook("admin/privacy/policies");
    ...
    <>
      <div className="p-6 bg-white min-h-screen m-2">
        ...
        {!data ? (
          ...
          <AddPolicy item={data?.data} refetch={refetch} />
        )}
      </div>
      <Modal title={"Add Office"}>
        <AddTerms close={() => setShowModal(false)} refetch={refetch} />
      </Modal>
      <View title="View">
        <ViewCertificate
