...
const AddTerms = ({ close, refetch, item }) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setContent(item.content ? JSON.parse(item.content) : [{ title: '', description: '' }]);
        ...
        {content.map((item, index) => (
          <div key={index} className="mt-2">
            <textarea
              ...
              onChange={(e) => handleUpdateField(index, 'title', e.target.value, setContent, content)}
            />
            <textarea
              className="border border-gray-400 w-full p-2 rounded mt-2"
              value={item.description}
              placeholder="item Description"
              onChange={(e) => handleUpdateField(index, 'description', e.target.value, setContent, content)}
            />
            <button
              type="button"
